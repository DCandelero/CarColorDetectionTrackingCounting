...
def get_car_counted(blobs_list, cars_counted, frame):
    ...
    for blob in blobs_list:
        ...
        if(blob_id not in cars_counted and blob_obj.crossed_counting_line):
            ...
            x = int(blob_obj.bounding_box[0] - (blob_obj.bounding_box[0]+blob_obj.bounding_box[2])*margin_x)
            y = int(blob_obj.bounding_box[1] - (blob_obj.bounding_box[1]+blob_obj.bounding_box[3])*margin_y)
            w = int(blob_obj.bounding_box[0]+blob_obj.bounding_box[2] + (blob_obj.bounding_box[0]+blob_obj.bounding_box[2])*margin_x)
            h = int(blob_obj.bounding_box[1]+blob_obj.bounding_box[3] + (blob_obj.bounding_box[1]+blob_obj.bounding_box[3])*margin_y)
